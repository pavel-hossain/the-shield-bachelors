...
interface DepositsPDFModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export const DepositsPDFModal: React.FC<DepositsPDFModalProps> = ({ isOpen, onClose }) => {
  const { currentPeriod, deposits, members, totalDepositsAmount } = useMess();
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      await generatePdfFromElement(
        reportRef.current,
        `Shield_Bachelors_Member_Deposits_History_${currentPeriod.label.replace(/\s+/g, '_')}.pdf`
      );
    } catch (err) {
      console.error('Deposits PDF Generation failed:', err);
    } finally {
      ...
      <div className="bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-800 w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh]">
        ...
        <div className="bg-slate-900 text-white p-3 sm:px-6 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-sm sm:text-base">Member Deposit Receipt History PDF</span>
          </div>
          <div className="flex items-center gap-2">
            <button
        ...
        <div className="p-4 sm:p-6 overflow-y-auto flex-1">
          ...
          >
            ...
            <div className="flex justify-between items-start border-b-2 border-slate-800 pb-4">
              <div>
                ...
                <p className="text-xs font-bold text-emerald-700 mt-1 uppercase tracking-wider">
                  MEMBER DEPOSIT RECEIPTS & CASH COLLECTION LOGS
                </p>
              </div>
            ...
            {/* Total Deposits Summary Box */}
            <div className="bg-emerald-50 border border-emerald-200 p-3 rounded-lg flex items-center justify-between">
              <div>
                <span className="text-xs font-bold text-emerald-800 uppercase block">Total Deposits Collected</span>
                <span className="text-[10px] text-slate-500">Includes Cash, bKash, Nagad, & Bank deposits</span>
              </div>
              <strong className="text-lg font-black text-emerald-900">৳ {totalDepositsAmount.toLocaleString()}</strong>
            </div>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead className="bg-slate-800 text-white font-bold">
                  <tr>
                    ...
                    <th className="py-2.5 px-3">Txn ID / Notes</th>
                  ...
                  {monthDeposits.map((d) => {
                    ...
                    return (
                      <tr key={d.id} className="hover:bg-slate-50">
                        <td className="py-2 px-3 font-medium text-slate-700 whitespace-nowrap">{d.date}</td>
                        <td className="py-2 px-3 font-bold text-slate-900 whitespace-nowrap">{member?.name || 'Unknown'}</td>
                        <td className="py-2 px-3 font-semibold text-emerald-700 whitespace-nowrap">{d.method}</td>
                        <td className="py-2 px-3 text-slate-500">
                          {d.transactionId && <span className="font-mono text-[10px] block font-bold text-slate-700">{d.transactionId}</span>}
                          {d.notes && <span className="text-[10px] italic">{d.notes}</span>}
                          {!d.transactionId && !d.notes && '-'}
                        </td>
                        <td className="py-2 px-3 text-right font-bold text-emerald-800 whitespace-nowrap">
                  ...
                  {monthDeposits.length === 0 && (
                    <tr>
                      <td colSpan={5} className="py-6 text-center text-slate-400 italic">
                        No member deposits logged for this month.
                      </td>
                    </tr>
                  )}
                </tbody>
