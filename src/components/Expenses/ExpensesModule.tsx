...
export const ExpensesModule: React.FC<ExpensesModuleProps> = ({
  ...
  const autoSuggestedCategory = suggestExpenseCategory(title);

  const handleTitleChange = (newTitle: string) => {
    setTitle(newTitle);
    const suggested = suggestExpenseCategory(newTitle);
              ...
              <div>
                <label className="text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center justify-between mb-1">
                  ...
                  {autoSuggestedCategory && (
                    <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 flex items-center gap-1 bg-emerald-50 dark:bg-emerald-950/80 px-2 py-0.5 rounded-md border border-emerald-200 dark:border-emerald-800 animate-pulse">
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Fish Bazar, Gas Cylinder, Electricity Bill, Cook Salary..."
                  value={title}
                ...
                <div className="flex items-center gap-1 mt-1.5 flex-wrap">
                  ...
                  {[
                    { text: 'Fish Bazar', cat: 'Market Shopping' },
                    { text: 'Gas Cylinder', cat: 'Gas' },
                    { text: 'Electricity Bill', cat: 'Utility' },
                    { text: 'Cook / Bua Salary', cat: 'Maid / Cook' },
                    { text: 'WiFi Internet', cat: 'Internet' },
                    { text: 'House Rent', cat: 'Rent' },
                  ].map((kw) => (
                    <button
                      key={kw.text}
                      type="button"
                      onClick={() => {
                        setIsCategoryManuallyChanged(false);
                        handleTitleChange(kw.text);
                      }}
                      className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 hover:bg-amber-100 dark:hover:bg-amber-950/80 text-slate-600 dark:text-slate-300 text-[10px] font-medium transition cursor-pointer"
                    >
                      +{kw.text}
                    </button>
                  ))}
