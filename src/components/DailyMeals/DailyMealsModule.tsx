...
import { Utensils, Calendar, Plus, Minus, Copy, Trash2, CheckCircle2, History, Search, FileText } from 'lucide-react';
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              {isManagerMode ? (
                <div className="flex items-center gap-2 overflow-x-auto w-full sm:w-auto">
                  <button
                    onClick={() => setAllMealsForDate(selectedDate, 0.5, 1, 1)}
                  ...
                  >
                    ...
                    <span>Set All Default (2.5 Meals)</span>
                  </button>
                  <button
                    onClick={() => copyPreviousDayMeals(selectedDate)}
                    className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-700 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition whitespace-nowrap"
                  >
                    <Copy className="w-3.5 h-3.5 text-indigo-500" />
                    <span>Copy Yesterday</span>
                  </button>
                  <button
                    onClick={() => clearMealsForDate(selectedDate)}
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {activeMembers.map((member) => {
                ...
                return (
                  <div
                    key={member.id}
                    className="p-3 sm:p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition"
                  >
                    {/* Member Details */}
                    ...
                    {/* Total Day Count Badge */}
                    <div className="flex sm:flex-col items-center justify-between sm:justify-center border-t sm:border-t-0 border-slate-100 dark:border-slate-800 pt-2 sm:pt-0 shrink-0">
                      <span className="text-[10px] font-semibold text-slate-400 sm:hidden">
                        Total Day Meals
                      </span>
                      <span className="px-3 py-1 bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300 rounded-lg text-xs font-black whitespace-nowrap">
